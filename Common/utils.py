...
def update_mean_var_count_from_moments(mean, var, count, batch_mean, batch_var, batch_count):
   ...
   #as more batches are taken in the mean is changed less
   # because tot_count approaches infinity asymptotically
   new_mean = mean + delta * batch_count / tot_count

    
   #the amount the current variance will contribute to variance calc
   m_a = var * count
   
   #the amount the batch variance will contribute to variance calc
   m_b = batch_var * batch_count

   #as we gather more batches, total count will again take over asymptotically
   #so earlier batches change the variance a lot more 
...
def explained_variance(ypred, y):
    ...
    interpretation:
        ev=0 => migh as well have predicted zero
...
class StickyActionEnv(gym.Wrapper):
    ...
    #a value between 0 and 1 is selected if the probability is less
    #we redo an action
    ...
    #Help, why is last action not a random action
    #How does sticky actions work when compound actions were taken?
    def reset(self):
        self.last_action = 0
